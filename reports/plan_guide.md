...
- **Описание**:  
  Некорректные ограничения прав пользователей, позволяющие доступ к чужим данным или функциям (например, просмотр чужого аккаунта через изменение `user_id` в URL).
- **Методы атак**:  
  - Перебор параметров в URL или API.  
  - Эскалация привилегий (например, доступ к админ-панели от обычного пользователя).  
- **Обнаружение**:  
  - Автоматизированное тестирование (например, OWASP ZAP).  
  - Ручная проверка прав через изменение параметров запросов.  
  - Анализ логов на подозрительные запросы (например, множественные `403 Forbidden`).  

---

### 2. Криптографические сбои (Cryptographic Failures)
- **Описание**:  
  Утечка данных из-за слабого шифрования или его отсутствия (например, передача паролей по HTTP).  
- **Методы атак**:  
  - Перехват трафика (MitM-атаки).  
  - Взлом слабых алгоритмов (например, SHA-1).  
- **Обнаружение**:  
  - Сканирование TLS/SSL (инструменты вроде [SSL Labs](https://www.ssllabs.com/)).  
  - Проверка наличия шифрования для конфиденциальных данных (например, токенов сессий).  

---

### 3. Инъекции (Injection)
- **Описание**:  
  Внедрение вредоносного кода через входные данные (SQL, OS-команды, XSS).  
- **Методы атак**:  
  - SQL-инъекции через формы поиска.  
  - XSS-атаки через комментарии на сайте.  
- **Обнаружение**:  
  - Сканеры уязвимостей (`SQLmap`, `Burp Suite`).  
  - Мониторинг логов на аномальные запросы (например, строки с `UNION SELECT`).  
...
- **Методы атак**:  
  - Эксплуатация логических уязвимостей (например, множественные бесплатные переводы).  
- **Обнаружение**:  
...
- **Методы атак**:  
  - Сканирование портов (`nmap`) для поиска уязвимых служб.  
- **Обнаружение**:  
  - Инструменты аудита (`Chef`, `Ansible`).  
...
- **Описание**:  
  Использование библиотек с известными CVE (например, Log4j).  
- **Методы атак**:  
  - Эксплойты для устаревших версий (например, Apache Struts).  
- **Обнаружение**:  
  - Сканирование зависимостей (`OWASP Dependency-Check`, `Snyk`).  
...
- **Описание**:  
  Подмена зависимостей или обновлений (например, через взлом npm-пакетов).  
- **Методы атак**:  
...
- **Методы атак**:  
  - Скрытие следов (например, очистка логов).  
- **Обнаружение**:  
...
- **Описание**:  
  Обход firewall через поддельные запросы от сервера (например, доступ к внутренней БД).  
- **Методы атак**:  
